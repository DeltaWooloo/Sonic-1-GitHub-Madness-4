...
; VRAM Base Addresses
_VRAM_PlaneA	= vram_fg
_VRAM_PlaneB	= vram_fg
_VRAM_Sprites	= vram_sprites
...
; Object variables
; NOTICE: This screen uses a custom objects format
render	= 1	; b	bitfield
art	= 2	; w	integer
maps	= 4	; l	pointer
xpos	= 8	; w	integer
...
; System Ports
VDP_Data	= vdp_data_port
VDP_Ctrl	= vdp_control_port

; Joypad virtual ports
Joypad		= v_jpadpress1

; Maincore misc variables
ArtBuffer	= v_256x256	; $9B00 bytes	Buffer used to store decompressed art
...
VBlankSub	= v_vbla_routine	; .b		Rotuine ID to execute during VBlank
HScroll		= v_hscrolltablebuffer	; $380 bytes	Horizontal scroll data
Objects		= v_objspace	; $2000 bytes	Objects RAM
Palette		= v_pal_dry	; $80 bytes	Actual screen palette
...
ObjLogo		ds.b	$40
ObjTitle	ds.b	$40
ObjSSRGSonic	ds.b	$40
		dephase

; Screen private memory
		phase	ScreenVars
QuadRadius	ds.w	1		; radius
QuadAngle	ds.w	1		; rotation angle (8 fixed)
QuadX		ds.l	1		; X-position (16 fixed)
QuadY		ds.l	1		; Y-position (16 fixed)
		dephase

; ---------------------------------------------------------------
; Direct copy data into VRAM via DMA
; ---------------------------------------------------------------
; Arguments:	source - Source Offset
;		len - Transfer Length (in bytes)
;		dest - Destination
; ---------------------------------------------------------------
SSRG_writeVRAM:	macro source,len,dest
...
NewSSRG_Screen:
	...
	jsr	PaletteWhiteOut		; fade palette to white
...
.ldtile	move.l	d0,(a5)			; create a solid tile (used for quad)
	...
	SSRG_writeVRAM ArtBuffer, $A6A0, _VRAM_Logo_St	; transfer it to VRAM via DMA
...
.ldpal	move.l	(a0)+,(a1)+
	...
	locVRAM	_VRAM_PlaneA,(a6)	; set start VRAM location
...
.pat2	move.l	d2,(a5)			; draw pattern #2
	...
	move.l	#SSRG_Obj_Quad,obj(a0)	; setup code pointer to handle this object
	...
	move.b	#$16,VBlankSub
	jsr	WaitForVBla		; send data to VDP
...
.loop	tst.b	(a0)			; is this slot occupied?
...
.l3:
	...
	; Calculate H/W (Val1)
	;tst.w	d5
	beq.s	.l0		; if W = 0, run away, or THE WORLD WILL FUCKIN EXPLODE, trust me
	moveq	#0,d2
	move.w	d4,d2		; d2 = H
	...
	; Calculate W/H (Val2)
	tst.w	d4
	beq.s	.l1		; if H = 0, run away, or THE WORLD WILL FUCKIN EXPLODE, trust me
.l0	moveq	#0,d3
	move.w	d5,d3		; d3 = W
...
.rotdone
	...
	jsr	QueueSound2		; play stomping sound
...
Logo_Intro:
	...
	move.l	#-$00044E00,xvel(a0)	; setup slowdown physics
	...
	jsr	QueueSound2		; play bumping sound
...
; ---------------------------------------------------------------
.SceneData:
	incbin	"_gamemode/#SSRG/SceneData.bin"
...
.s0	cmpi.w	#$C1,xpos(a0)		; has Sonic reached $C1 on X-axis
	scc	d0
	tst.w	yvel(a0)		; is Y-vel negative?
	smi	d1
	and.b	d0,d1
...
.setjump
	...
	jsr	QueueSound2		; play ultra sound
...
SSRG_Palette:
	incbin	"_gamemode/#SSRG/Sonic_Pt1.pal"		; row 0
	incbin	"_gamemode/#SSRG/Sonic_Pt2.pal"		; row 1
	incbin	"_gamemode/#SSRG/Logo.pal"		; row 2
	incbin	"_gamemode/#SSRG/Title.pal"		; row 3
...
SSRG_LogoMaps:
	incbin	"_gamemode/#SSRG/LogoMaps.bin.kos"
	even
...
SSRG_Art_Sonic:
	incbin	"_gamemode/#SSRG/Sonic.4bpp.nem"
	even

SSRG_Art_Title:
	incbin	"_gamemode/#SSRG/Title.4bpp.nem"
	even

SSRG_Logo_Bank1:
	incbin	"_gamemode/#SSRG/Logo_Bnk1.4bpp.kos"

SSRG_Logo_Bank2:
	incbin	"_gamemode/#SSRG/Logo_Bnk2.4bpp.kos"
	even
