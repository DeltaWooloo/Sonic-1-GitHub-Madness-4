; ---------------------------------------------------------------------------
; Object 2D - Burrobot enemy (LZ) - now practically a copy of deltas hammer bro but mainly a boomerang now
; ---------------------------------------------------------------------------
...
; ===========================================================================
Burro_Index:	dc.w Burro_Main-Burro_Index	; Routine 0 - init, runs once
		dc.w Burro_Walk-Burro_Index	; Routine 2 - main walking loop
		dc.w Burro_Delete-Burro_Index	; Routine 4 - destroyed by Sonic

Burro_timer    = objoff_30	; frames until next direction change (word)
Burro_throwtmr = objoff_32	; frames until next hammer throw (word)
Burro_jumptmr  = objoff_34	; frames until next jump (word)
; ===========================================================================

Burro_Main:	; Routine 0
		addq.b	#2,obRoutine(a0)	; call Burro_Walk while object is being initialised
		move.b	#$1C,obHeight(a0)	; set height before floor snap
		move.b	#$A,obWidth(a0)
		move.l	#Map_Len,obMap(a0)	; point to our mappings table
...
.notfuckedupbro:
		...
		move.w	#180,Burro_timer(a0)	; walk for 3 seconds before turning (180 frames)
		move.w	#90,Burro_throwtmr(a0)	; throw first hammer after 1.5 seconds (90 frames)
		move.w	#150,Burro_jumptmr(a0)	; first jump after 2.5 seconds (150 frames)
		
Burro_Walk:	; Routine 2
		moveq	#0,d0
		move.b	ob2ndRout(a0),d0
		move.w	.index(pc,d0.w),d1
		jsr	.index(pc,d1.w)
		lea	(Ani_Burro).l,a1		; load animation table
...
.walk:
		; --- Direction timer ---
		subq.w	#1,Burro_timer(a0)	; count down the walk timer
		bpl.s	.chkjump		; still time left, check jump next
		move.w	#180,Burro_timer(a0)	; reset timer to 3 seconds (180 frames)
		neg.w	obVelX(a0)		; flip velocity to reverse direction
		move.b	#1,obAnim(a0)
.chkjump:
		; --- Jump timer ---
		subq.w	#1,Burro_jumptmr(a0)	; count down the jump timer
		bpl.s	.chkthrow		; still time left, check throw next
		move.w	#180,Burro_jumptmr(a0)	; reset timer to 3 seconds (180 frames)
		move.b	#2,obAnim(a0)
...
.chkthrow:
		; --- Throw timer ---
		subq.w	#1,Burro_throwtmr(a0)	; count down the throw timer
		bpl.s	.move			; still time left, continue moving
		move.w	#90,Burro_throwtmr(a0)	; reset timer to 1.5 seconds (90 frames)
		bra.w	Burro_ThrowHammer	; spawn a hammer projectile
